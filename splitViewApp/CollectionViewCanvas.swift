...
class CollectionViewCanvas: UICollectionViewController, UICollectionViewDelegateFlowLayout  {
    ...
    override func viewDidLoad() {
        ...
        logMessage = "Created log at: \(NSDate().timeIntervalSince1970-1493000000)\n\n"
    ...
    func useCircledCell(center: CGPoint) {
        ...
        if indexPath != nil && index > 12 && !selectedIndices.isEmpty {
            ...
            // add to log
            logMessage.appendContentsOf("Copied \(valueToInsert) to cell \(index) with circle gesture: \(NSDate().timeIntervalSince1970-1493000000)\n\n")
        }
    }
    ...
    func useSelectedCell(indexPathIn: NSIndexPath) {
        ...
        if index > 191 && index < 203 && cell.label.text != "" {
            ...
            if op == "+" {
                ...
                logMessage.appendContentsOf("Addition: \(NSDate().timeIntervalSince1970-1493000000)\n\n")
            }
            if op == "-" {
                arithmeticValue = Double(table.getTableItem(selectedIndices[0].item))! - Double(table.getTableItem(selectedIndices[1].item))!
                selectedIndices.removeAll()
                logMessage.appendContentsOf("Subtraction: \(NSDate().timeIntervalSince1970-1493000000)\n\n")
            }
            if op == "*" {
                arithmeticValue = Double(table.getTableItem(selectedIndices[0].item))! * Double(table.getTableItem(selectedIndices[1].item))!
                selectedIndices.removeAll()
                logMessage.appendContentsOf("Multiplication: \(NSDate().timeIntervalSince1970-1493000000)\n\n")
            }
            if op == "/" {
                arithmeticValue = Double(table.getTableItem(selectedIndices[0].item))! / Double(table.getTableItem(selectedIndices[1].item))!
                selectedIndices.removeAll()
                logMessage.appendContentsOf("Division: \(NSDate().timeIntervalSince1970-1493000000)\n\n")
            }
            if op == "∑" {
                var temp = 0.0
                ...
                selectedIndices.removeAll()
                logMessage.appendContentsOf("Summation: \(NSDate().timeIntervalSince1970-1493000000)\n\n")
            }
            if op == "Sort ↑" {
                sortSelectedIndices(true)
                selectedIndices.removeAll()
                logMessage.appendContentsOf("Sort ascending: \(NSDate().timeIntervalSince1970-1493000000)\n\n")
            }
            if op == "Sort ↓" {
                sortSelectedIndices(false)
                selectedIndices.removeAll()
                logMessage.appendContentsOf("Sort descending: \(NSDate().timeIntervalSince1970-1493000000)\n\n")
            }
        ...
        if let selection = selectedIndices.indexOf(indexPathIn) {
            ...
            setupOperatorCells()
            logMessage.appendContentsOf("Deselection: \(NSDate().timeIntervalSince1970-1493000000)\n\n")
            return
        }
            ...
            self.collectionView!.reloadItemsAtIndexPaths(indicesToUpdate)
            setupOperatorCells()
            logMessage.appendContentsOf("Selection: \(NSDate().timeIntervalSince1970-1493000000)\n\n")
        }
    ...
    func didCrossOutCell(indexPath: NSIndexPath) {
        ...
        logMessage.appendContentsOf("Deleted cell with value \(cell.label.text): \(NSDate().timeIntervalSince1970-1493000000)\n\n")
    ...
    @IBAction func clearAll(sender: UIButton) {
        ...
        popover?.sourceRect = CGRectMake(100, 920, 300, 40)
        popover?.permittedArrowDirections = UIPopoverArrowDirection.Any
        
        // present the alert controller
        presentViewController(ac, animated: true, completion: nil)
        
        logMessage.appendContentsOf("Clear: \(NSDate().timeIntervalSince1970-1493000000)\n\n")
    }
    ...
    @IBAction func showHideLog(sender: UIButton) {
        if !textView.hidden {
            ...
            logMessage.appendContentsOf("Opened log at: \(NSDate().timeIntervalSince1970-1493000000)\n\n")
